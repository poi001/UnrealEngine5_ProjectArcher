...
#include "PA_Enemy_Character_Goblin.h"
...
APA_Enemy_Character_Goblin::APA_Enemy_Character_Goblin()
{
	//기본 애님 인스턴스 설정
	static ConstructorHelpers::FClassFinder<UPA_Enemy_AnimInstance> ENEMY_ANIM(TEXT("/Game/Character/Enemy/AnimBP/AnimBP_Goblin.AnimBP_Goblin_C"));
...
void APA_Enemy_Character_Goblin::PostInitializeComponents()
{
	Super::PostInitializeComponents();

	EnemyAnimInstance = Cast<UPA_Enemy_AnimInstance>(GetMesh()->GetAnimInstance());
	//공격 판정 델리게이트 설정
	EnemyAnimInstance->OnAttackCheckDelegate.AddUObject(this, &APA_Enemy_Character_Goblin::AttackCheck);
	//공격 끝날 시 델리게이트 설정
	EnemyAnimInstance->OnAttackEndDelegate.AddUObject(this, &APA_Enemy_Character_Goblin::AttackEnd);
}

void APA_Enemy_Character_Goblin::PossessedBy(AController* NewController)
{
...
void APA_Enemy_Character_Goblin::AttackEnd()
{
